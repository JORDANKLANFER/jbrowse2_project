...
# Ensure in jbrowse2 directory  
# Run script in terminal: ./fetch_and_process_data.sh
#if permission issues: chmod +x fetch_and_process_data.sh

jbrowse add-track data/siv_data/siv_sorted_annotations.gff.gz --assemblyNames SIV_Reference_Genome --load inPlace
# ------------------------------------------------------------------------

jbrowse add-assembly data/hiv1_data/hiv1_genome.fna --load inPlace
# checks in correct directory
cd "$(dirname "$0")"

# creates/checks for main data folder 
if [[ ! -d "data" ]]; then
    mkdir -p ./data
fi
